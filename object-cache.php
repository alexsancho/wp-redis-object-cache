 ...
 * @see https://github.com/ericmann/Redis-Object-Cache
 */
...
class WP_Object_Cache {
	...
	protected function add_or_replace( $add, $key, $value, $group = 'default', $expiration = 0 ) {
		...
		if ( ! in_array( $group, $this->ignored_groups, false ) && $this->redis_status() ) {
	 ...
	 * Invalidate all items in the cache.
	 *
	 * @param int|float $delay Number of seconds to wait before invalidating the items.
	 *
	 * @return bool Returns TRUE on success or FALSE on failure.
	 */
	 ...
	 * @param string      $key    The key under which to store the value.
	 * @param string      $group  The group value appended to the $key.
	 * @param string|bool $force  Optional. Whether to force a refetch rather than relying on the local cache. Default false.
	 ...
	 * @param   mixed     $value      The value to store.
	 * @param   string    $group      The group value appended to the $key.
	 * @param   int|float $expiration The expiration time, defaults to 0.
	 *
	 * @return  bool               Returns TRUE on success or FALSE on failure.
	 */
	 ...
	 *
	 * @param string|bool $value Value to possibly convert
	 *
	 * @return string|bool   Converted value
	 */
	...
	protected function validate_expiration( $expiration ) {
		$expiration = ( is_array( $expiration ) || is_object( $expiration ) ? 0 : abs( (int) $expiration ) );
		if ( $expiration === 0 && defined( 'WP_REDIS_MAXTTL' ) ) {
			$expiration = (int) WP_REDIS_MAXTTL;
		}
